...
func (s *Strategy) ProcessIDToken(r *http.Request, provider Provider, idToken, idTokenNonce string) (*Claims, error) {
	...
	// Preserve the fork's legacy native ID-token nonce behavior. Provider token
	// verification and claim validation above still apply. Restoring nonce
	// enforcement requires a separately tested client migration, not a rebase.
